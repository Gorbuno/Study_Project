...
int main() {
    ...
    sigemptyset(&signmask); /*инициализируем набор signmask, обнуляем (удаляем ранее в нем находящиеся сигналы) */
    sigaddset(&signmask, SIGTTIN);/* добавляем сигнал ввода в наборы signmask*/
	...
	if (sigprocmask(SIG_BLOCK, &signmask, NULL) == -1) {/* добавляем набор signmask в список блокируемых сигналов, что бы отследить момент когда он измениться (не очень умно, но так работает. сейчас пнимаю что можно было бы делать по другому)*/ 
		perror("sigprocmask");
		exit(EXIT_FAILURE);
	}
